...
client.prototype.handleMessage = function handleMessage(message) {
	...
	if(message.tags) {
		...
		for(var key in tags) {
			...
			} else if (key !== "emote-sets" && key !== "ban-duration" && key !== "bits") {
		...
		switch(message.command) {
			...
			case "372":
				...
				for (var i = 0; i < joinChannels.length; i++) {
					...
					joinQueue.add(() => {
						if(!_.isNull(this.ws) && this.ws.readyState === 1) {
							this.join(channel).catch(err => {
								this.log.error("join " + channel);
								this.log.error(err);
							});
						}
					});
			...
			case "RECONNECT":
				...
				setTimeout(() => this.connect().catch(err => {
					this.log.error("setTimeout reconnect connect");
					this.log.error(err);
				}), this.reconnectTimer);
				break;
	...
	if(this.reconnect && !this.reconnecting && this.reconnections <= this.maxReconnectAttempts-1) {
		...
		setTimeout(() => {
			...
			this.connect().catch(err => {
				this.log.error("setTimeout connect");
				this.log.error(err);
			});
		}, this.reconnectTimer);
	}
...
client.prototype._onClose = function _onClose(e) {
	this.log.error('ws _onClose');
	console.log(e);
...
client.prototype._sendCommand = function _sendCommand(delay, channel, command, fn) {
	...
	return new Promise((resolve, reject) => {
		...
		if(_.isNull(this.ws) || this.ws.readyState !== 1) {
			...
			_.promiseDelay(delay).then(() => reject(`No response from Twitch. (${channel} ${command})`));
		}
